...
def get_edge_mask_link_negative(mask_link_positive, num_nodes, num_negtive_edges):
    ...
    for i in range(num_negtive_edges):
        ...
        count = 0
        while True:
            mask_temp = tuple([random.randint(0,num_nodes-1),random.randint(0,num_nodes-1)])
...
def resample_edge_mask_link_negative(data):
    #sorted_mask_pos = sorted(data.mask_link_positive_train)
...
def deduplicate_edges(edges):
    edges_new = np.zeros((2,edges.shape[1]), dtype=int)
    # add none self edge
    j = 0
...
def get_dist_max(anchorset_id, dist, device):
    ...
    for i in range(len(anchorset_id)):
        ...
        dist_max_temp, dist_argmax_temp = torch.zeros(dist_temp.shape[0]), torch.zeros(dist_temp.shape[0])
        """for i in range(dist_temp.shape[0]):
            if i % 1000 == 0:
                print(i)
            array = dist_temp[i].toarray()
            tens = torch.tensor(array)
            part_max, part_arg = torch.max(tens, dim=-1)
            dist_max_temp[i] = part_max
            dist_argmax_temp[i] = part_arg"""
        dist_max_temp = torch.rand(dist_temp.shape[0])
        dist_argmax_temp = torch.randint(0, dist_temp.shape[0]-1,size=(dist_temp.shape[0],))
...
def culculate_acc(pred, label):
    temp_pred = []
    out_act = nn.Sigmoid()
    pred = out_act(pred)
    temp_label = label.data.cpu().numpy()
    for i in range(len(pred)):
        if pred[i] >= 0.5:
            temp_pred.append(1)
        else:
            temp_pred.append(0)
    return sklearn.metrics.accuracy_score(temp_label, temp_pred)