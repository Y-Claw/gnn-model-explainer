...
class GCN(torch.nn.Module):
    def __init__(self, input_dim, feature_dim, hidden_dim, output_dim,
                 feature_pre=True, layer_num=2, dropout=True, act=F.relu, **kwargs):
        ...
        if feature_pre:
            ...
            self.conv_first = tg.nn.GCNConv(input_dim, hidden_dim, normalize=False)
        self.conv_hidden = nn.ModuleList([tg.nn.GCNConv(hidden_dim, hidden_dim, normalize=False) for i in range(layer_num - 2)])
        self.conv_out = tg.nn.GCNConv(hidden_dim, output_dim, normalize=False)
        self.act = act

        for m in self.modules():
            if isinstance(m, tg.nn.GCNConv):
                init.xavier_uniform_(m.weight.data, gain=nn.init.calculate_gain("relu"))
                #init.xavier_uniform_(m.weight.data, gain=nn.init.calculate_gain("sigmoid"))
                #m.weight.data = m.weight.data / 4

    def get_embedding(self, data, explain=False):
        ...
        if self.dropout:
            x = F.dropout(x, training=self.training)
        for i in range(self.layer_num - 2):
            x = self.conv_hidden[i](x, edge_index, edge_weight=edge_weight)
            x = self.act(x)
            if self.dropout:
                x = F.dropout(x, training=self.training)
        x = self.conv_out(x, edge_index, edge_weight=edge_weight)
        return x
...
class PGNN(torch.nn.Module):
    ...
    def forward(self, data, explain=None):
        x = data.x
        if self.feature_pre:
            x = self.linear_pre(x)
